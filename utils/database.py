    ...
    with self.get_connection() as conn:
        try:
            with conn.cursor() as cur:
                ...
                cur.execute(r"""
                    ...
                    mapped AS (
                      SELECT r.job_vacancy_id,
                             r.selected_id_raw,
                             -- try exact match first (same format), then try with 'EMP' prefix,
                             -- then try substring match against employees.employee_id as last resort
                    ...
                    FROM mapped
                """)
                # After this, tb_selected_talents has:
                # job_vacancy_id | selected_id_raw | mapped_employee_id (nullable)

                # 4. Profiles psych norm (same)
                cur.execute("""
                ...
                # 5. TV scores long (same as before)
                cur.execute(r"""
                ...
                # Use mapped_employee_id from tb_selected_talents; fall back to selected_id_raw if mapped found nothing
                cur.execute(r"""
                ...
                # 7. Employees with TV (same but safe)
                cur.execute("""
                ...
                # 13. FINAL SELECT - RETURN WITH ALL NECESSARY COLUMNS (same)
                final_sql = """
                SELECT
                  tm.employee_id,
                  tm.fullname,
