...
class DatabaseManager:
    ...
    def run_matching_query(self, job_vacancy_id: int) -> pd.DataFrame:
        ...
        """
        # use connection context manager so conn is closed automatically
        with self.get_connection() as conn:
            try:
                # use a cursor context manager too
                with conn.cursor() as cur:
                    # ===== 1) DROP OLD TABLES =====
                    cur.execute(
                        """
                    ...
                    # Use raw string (r"""...""") for SQL that contains backslashes
                    cur.execute(
                        r"""
                    ...
                    # read results into pandas DataFrame and return
                    df = pd.read_sql_query(final_sql, conn)
                    return df

            # end with conn.cursor()
            # any exception from above will be catched here
            except Exception as e:
                # rollback if possible and report
                try:
                    conn.rollback()
                except Exception:
