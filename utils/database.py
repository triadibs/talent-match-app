...
class DatabaseManager:
  ...
  def run_matching_query(self, job_vacancy_id: int) -> pd.DataFrame:
    """
    Revised matching pipeline:
    - compute baseline mean & stddev per TV using selected benchmarks
    - compute z-score per employee per TV: (user_score - baseline_mean) / NULLIF(baseline_stddev,0)
    - transform via logistic to 0..100: 100 * (1 / (1 + exp(-z)))
    - fallback to ratio method when stddev is NULL/0 or baseline_mean is NULL
    - aggregate TGV (weighted), then final aggregation
    ...
    with self.get_connection() as conn:
        try:
            with conn.cursor() as cur:
                ...
                # 2) latest year (used by competencies queries)
                cur.execute(
                    """
                ...
                # 4) profiles psych norm (same as before)
                cur.execute(
                    """
                ...
                # 5) tb_tv_scores_long (unchanged logic, keep unions for all tv sources)
                cur.execute(r"""
                    ...
                    WHERE c.year = ly.max_year AND c.score IS NOT NULL AND trim(coalesce(c.score::text,'')) <> '' AND (c.score::text ~ '^\s*[-+]?\d+(\.\d+)?\s*$')
                ...
                # 6) tb_baseline_per_tv: compute mean and stddev from selected talents (NOT median)
                cur.execute(
                    """
                ...
                # 7) employees with tv (join tv scores and baseline stats)
                cur.execute(
                    """
                ...
                # 8) compute per-TV match using z-score => logistic transform (and fallback to ratio when stddev is null/zero)
                # logistic_score = 100 * (1 / (1 + exp(-z))) where z = (user_score - baseline_mean) / NULLIF(baseline_stddev, 0)
                # If baseline_mean is null (no baseline), we set tv_match_score = NULL
                cur.execute(
                    """
                    ...
                    SELECT
                      ...
                      CASE
                        ...
                        WHEN e.baseline_mean IS NOT NULL AND e.baseline_stddev IS NULL THEN
                          -- fallback: ratio vs baseline mean (preserve direction)
                          CASE
                ...
                # 10) tgv aggregation (weighted average of tv_match_rate)
                cur.execute(
                    """
                ...
                # 11) tgv with weights (use defaults if weights_config missing)
                cur.execute(
                    r"""
                ...
                # 12) final aggregation: weighted average of tgv_match_rate
                cur.execute(
                    """
                ...
                # ensure numeric scale: final_match_rate already 0..100 from logistic/ratio
                return df
