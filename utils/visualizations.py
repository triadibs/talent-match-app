# utils/visualizations.py (revised)
# -*- coding: utf-8 -*-
"""
...
Includes small fallbacks and tolerant comparisons for employee_id matching.
"""
...
def plot_tgv_radar(results_df: pd.DataFrame, employee_id: str) -> go.Figure:
    ...
    # Filter safely by employee_id (accept numeric or str) and strip whitespace
    emp_mask = df['employee_id'].astype(str).str.strip() == str(employee_id).strip()
...
def _derive_tv_match_rate_fallback(emp_df: pd.DataFrame) -> pd.Series:
    """
    Fallback scaler for tv_score -> tv_match_rate (0..100) using min-max per tv_name in emp_df.
    This is used only when tv_match_rate is missing but tv_score exists within the provided subset.
    """
    df = emp_df.copy()
    if 'tv_score' not in df.columns:
        return pd.Series(np.nan, index=df.index)

    # simple min-max scaling per tv_name
    out = pd.Series(index=df.index, dtype=float)
    if 'tv_name' in df.columns:
        for name, gidx in df.groupby('tv_name').groups.items():
            s = pd.to_numeric(df.loc[gidx, 'tv_score'], errors='coerce')
        ...
        s = pd.to_numeric(df['tv_score'], errors='coerce')
        mn = s.min(skipna=True)
        mx = s.max(skipna=True)
        if pd.isna(mn) or pd.isna(mx):
            out[:] = np.nan
...
def plot_tv_heatmap(results_df: pd.DataFrame, employee_id: str, top_tgv_count: int = 4) -> go.Figure:
    ...
    # Accept datasets that have tv_score instead of tv_match_rate and derive fallback if needed
    if results_df is None or results_df.empty:
        raise ValueError("results_df is empty or None")

    # Ensure minimal columns
    if not {'employee_id', 'tgv_name', 'tv_name'}.issubset(results_df.columns):
        raise ValueError("results_df must contain at least 'employee_id', 'tgv_name', and 'tv_name' columns")

    df = _ensure_str_cols(results_df, ['employee_id', 'tgv_name', 'tv_name'])
    emp_df = df[df['employee_id'].astype(str).str.strip() == str(employee_id).strip()].copy()
    if emp_df.empty:
        raise ValueError(f"No rows for employee_id={employee_id} in results_df")

    # If tv_match_rate missing but tv_score exists, derive fallback for this employee subset
    if 'tv_match_rate' not in emp_df.columns and 'tv_score' in emp_df.columns:
        emp_df['tv_match_rate'] = _derive_tv_match_rate_fallback(emp_df)

    if 'tv_match_rate' not in emp_df.columns:
        # attempt other columns generically
        for alt in ['tv_score', 'score', 'match_score']:
            if alt in emp_df.columns:
                emp_df['tv_match_rate'] = _to_numeric_percent(emp_df[alt])
                break

    emp_df['tv_match_rate'] = _ensure_percent_scale(emp_df.get('tv_match_rate'))
...
def plot_strengths_gaps(results_df: pd.DataFrame, employee_id: str, top_k: int = 10) -> go.Figure:
    ...
    # require tv_name present; tv_match_rate may be derived
    if 'tv_name' not in results_df.columns:
        raise ValueError("results_df must contain column 'tv_name'")
    ...
    # derive tv_match_rate if missing
    if 'tv_match_rate' not in emp_df.columns and 'tv_score' in emp_df.columns:
        emp_df['tv_match_rate'] = _derive_tv_match_rate_fallback(emp_df)
