...
- coerces numeric-like strings to floats
- supports percent strings and 0..1 vs 0..100 scaling
- avoids .str accessor errors by converting to string-safe representations first
- raises clear ValueError when input lacks required columns or numeric values
"""
...
def _to_numeric_percent(series: pd.Series) -> pd.Series:
    ...
    Steps:
      ...
      - remove percent sign, commas, parentheses
      - coerce to numeric
      - if numeric max <= 1.0 assume 0..1 scale and multiply by 100
    Returns float Series (may contain NaN).
    """
    if series is None:
    ...
    # Keep NaN as NaN, convert other values to stripped string
    ...
    # Now safe to use .str operations (only on non-null entries)
    # Remove percent sign and thousand separators; normalize comma decimal to dot
    # Note: these operate on string values; NaN remain NaN
    s = s.where(s.isna(), s.str.replace("%", "", regex=False))
    s = s.where(s.isna(), s.str.replace(",", ".", regex=False))
    s = s.where(s.isna(), s.str.replace(r"[()]", "", regex=True))

    coerced = pd.to_numeric(s, errors="coerce").astype(float)

    # If no numeric values, return as-is
...
# alias for clarity
safe_percent_to_float = _to_numeric_percent
...
def _ensure_str_cols(df: pd.DataFrame, cols: List[str]) -> pd.DataFrame:
    ...
    Returns a shallow copy of df with transformations applied.
    """
    df = df.copy()
    for c in cols:
        if c in df.columns:
            # keep NaN as NaN, convert other values to str when not already str
            df[c] = df[c].where(~df[c].isna(), other=np.nan)
...
def plot_match_distribution(summary_df: pd.DataFrame) -> go.Figure:
    ...
    x_series = safe_percent_to_float(summary_df["final_match_rate_percentage"]).dropna()
    if x_series.empty:
...
def plot_top_candidates(summary_df: pd.DataFrame, top_n: int = 10) -> go.Figure:
    ...
    df["final_match_rate_percentage"] = safe_percent_to_float(df["final_match_rate_percentage"])
...
def plot_tgv_radar(results_df: pd.DataFrame, employee_id: str) -> go.Figure:
    ...
    # coerce tgv_match_rate to numeric percent (0..100)
    emp_df["tgv_match_rate"] = safe_percent_to_float(emp_df["tgv_match_rate"])
...
def plot_tv_heatmap(results_df: pd.DataFrame, employee_id: str, top_tgv_count: int = 4) -> go.Figure:
    ...
    # ensure pivot uses string labels
