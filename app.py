...
def derive_tv_match_rate_from_tv_score(df: pd.DataFrame) -> pd.DataFrame:
    """
    Derive column 'tv_match_rate' in 0..100 from 'tv_score' and 'scoring_direction'.
    Uses min-max scaling per tv_name (safe for heterogeneous score ranges).
    """
    df = df.copy()
    if 'tv_score' not in df.columns:
        return df

    # Normalize employee_id to stripped string
    ...
    # Ensure scoring_direction present
    if 'scoring_direction' not in df.columns:
        df['scoring_direction'] = 'higher_better'
    ...
    def _scale_group(g):
        ...
        if mx == mn:
            # constant values: map to 100 for higher_better, 0 for lower_better
    ...
    df['tv_match_rate'] = pd.to_numeric(df.get('tv_match_rate'), errors='coerce').clip(lower=0.0, upper=100.0)
    return df
...
def normalize_match_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    - final_match_rate_percentage (0..100)
    - tgv_match_rate (0..100)
    - tv_match_rate (0..100) derived if tv_score present
    - employee_id as stripped string
    """
    df = df.copy()
    ...
    # final match percent: derive from final_match_rate if missing
    if 'final_match_rate_percentage' not in df.columns and 'final_match_rate' in df.columns:
    ...
    # normalize tgv_match_rate if present
    if 'tgv_match_rate' in df.columns:
        df['tgv_match_rate'] = pd.to_numeric(df['tgv_match_rate'], errors='coerce')
        if not df['tgv_match_rate'].dropna().empty and df['tgv_match_rate'].max(skipna=True) <= 1.0:
            df['tgv_match_rate'] = df['tgv_match_rate'] * 100.0

    # derive tv_match_rate from tv_score if needed
    if 'tv_match_rate' not in df.columns and 'tv_score' in df.columns:
        df = derive_tv_match_rate_from_tv_score(df)

    # final safety coercions
    if 'tv_match_rate' in df.columns:
        df['tv_match_rate'] = pd.to_numeric(df['tv_match_rate'], errors='coerce').clip(lower=0.0, upper=100.0)
...
<style>
    ...
    .success-box { background-color: #d4edda; border: 1px solid #c3e6cb; padding: 1rem; border-radius: 0.5rem; margin: 1rem 0; }
</style>
    ...
    with st.form("vacancy_form"):
        ...
        with col2:
            ...
            if not isinstance(high_performers_df, pd.DataFrame) or high_performers_df.empty:
                ...
                id_to_label = { str(row['employee_id']): f"{row['employee_id']} - {row['fullname']} ({row.get('position','')})"
                                for _, row in high_performers_df.iterrows() }
                selected_ids = st.multiselect("Select Benchmark Employees *",
                                              options=id_list,
        ...
        if submitted:
            ...
            # Save to session (with normalization)
...
def show_results_page():
    ...
    if summary_df is None or (isinstance(summary_df, pd.DataFrame) and summary_df.empty):
        st.error("matching_results_summary is missing or empty.")
        return
...
def show_analytics_page():
    ...
    # normalize again before analytics (idempotent)
    detailed = normalize_match_columns(detailed)
    st.session_state.matching_results_detailed = detailed

    # build summary if missing/empty
    ...
    try:
        ...
        tgv_summary['final_match_rate_percentage'] = pd.to_numeric(tgv_summary.get('final_match_rate_percentage'), errors='coerce')
    ...
    # format_func uses summary values but selectbox value remains employee_id
    def _format_emp(eid):
        row = tgv_summary[tgv_summary['employee_id'] == eid]
        if row.shape[0] == 0:
            return str(eid)
        name = row['fullname'].iloc[0]
        pct = row['final_match_rate_percentage'].iloc[0]
        try:
            pct_str = f"{float(pct):.1f}%"
        except Exception:
            pct_str = "N/A"
        return f"{eid} - {name} ({pct_str})"
